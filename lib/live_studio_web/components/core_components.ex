defmodule LiveStudioWeb.CoreComponents do
  @moduledoc """
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(LiveStudioWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(LiveStudioWeb.Gettext, "errors", msg, opts)
    end
  end
            ...
            <span class="flex min-w-0 items-center justify-between space-x-3">
              <%= for img <- @img do %>
                <%= if img.src do %>
                  ...
                  />
                <% else %>
                  <.icon name={img.default_icon} class="w-10 h-10" />
                <% end %>
              <% end %>
